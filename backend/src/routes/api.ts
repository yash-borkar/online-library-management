...
apiRouter.post("/auth/mfa", async (req: Request, res: Response) => {
  const bodySchema = z.object({
...
apiRouter.post("/books", async (req: Request, res: Response) => {
  ...
  const isbn =
    body.isbn && body.isbn.trim().length > 0 ? body.isbn.trim() : null;
  const category = (body.category?.trim() || "General").slice(0, 80);

  let coverImageUrl =
    body.coverImageUrl && body.coverImageUrl.trim().length > 0
  ...
  if (isbn) {
    ...
    if (dup) return jsonError(res, "ISBN already exists", 409);
  }
...
apiRouter.delete("/books/:id", async (req: Request, res: Response) => {
  ...
  if (!id) return jsonError(res, "Invalid id", 400);
...
apiRouter.get("/borrows", async (req: Request, res: Response) => {
  const u = await requireUser(req, res, [
    Role.ADMIN,
    Role.LIBRARIAN,
    Role.MEMBER,
...
apiRouter.post("/borrows", async (req: Request, res: Response) => {
  ...
  if (!u.mfaEnabled) {
    return jsonError(
      res,
      "Members must enable MFA before borrowing books",
      403,
    );
  }
  ...
  let body: z.infer<typeof borrowSchema>;
  try {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (!book) return { err: "not_found" as const };
    if (book.copiesAvailable < 1) return { err: "unavailable" as const };
    ...
    if (existing) return { err: "already" as const };
  ...
  if (result.err === "not_found") return jsonError(res, "Book not found", 404);
...
apiRouter.post("/mfa/setup", async (req: Request, res: Response) => {
  const u = await requireUser(req, res, [Role.MEMBER]);
  if (!u) return;
...
apiRouter.post("/mfa/confirm", async (req: Request, res: Response) => {
  const u = await requireUser(req, res, [Role.MEMBER]);
  if (!u) return;
